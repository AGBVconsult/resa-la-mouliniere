...
function extractLanguages(md: string): string[] {
  // Format: - `language: "fr" | "nl" | "en" | "de" | "it" | "es" | "be"`
  const lineMatch = md.match(/^\s*-\s+`language:\s*(.+?)`\s*$/m);
  if (!lineMatch) return ["fr", "nl", "en", "de", "it"];
  const langs: string[] = [];
  const re = /"([^"]+)"/g;
  for (const m of lineMatch[1].matchAll(re)) langs.push(m[1]);
  return langs.length > 0 ? langs : ["fr", "nl", "en", "de", "it"];
}
